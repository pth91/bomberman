...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if type(old_f) == type(state_to_features(new_game_state)):
        ...
        if action_num == 5 and np.sum(old_f[2:6] == 1) >= 1:
            events.append('BOMB_AT_CRATE')            
        if old_f[6] <= 7 and old_f[6] -4 == action_num and old_f[6] >= 4:
            events.append('TO_CRATE') # moved to crate
        if old_f[1] == 5 and old_f[6] <= 7 and old_f[6] -4 != action_num and old_f[6] >= 4:
            events.append('NOT_TO_CRATE') #moved not to crate while a crate is reachable
        if old_f[7] == 4 and old_f[0] == 1:
            events.append('ATTACK_OPP')
        if old_f[7] <= 3 and old_f[7] == action_num and old_f[6] == 8:
            events.append('TO_OPP')
        if old_f[7] <= 3 and old_f[7] != action_num and old_f[6] == 8:
            events.append('NOT_TO_OPP')
...
def q_update_transitions(self):
    ...
    for i in range(len(self.transitions)):
            ...
            for j in range(8):
                self.model[old[0], old[1], old[2], old[3], old[4], old[5], old[6], old[7], action_num] = (1 - alpha) * self.model[old[0], old[1], old[2], old[3], old[4], old[5], old[6], old[7], action_num] + 0.5 * alpha * (reward + np.max(self.model[new[0], new[1], new[2], new[3], new[4], new[5], new[6], new[7]]))
...
def rotate_features(old, new, action):
    old = old[[0, 1, 4, 5, 3, 2, 6, 7]]
    new = new[[0, 1, 4, 5, 3, 2, 6, 7]]
    old[1] = [1, 2, 3, 0, 4, 5][old[1]]
    new[1] = [1, 2, 3, 0, 4, 5][new[1]]
    old[6] = [1, 2, 3, 0, 5, 6, 7, 4, 8][old[6]]
    new[6] = [1, 2, 3, 0, 5, 6, 7, 4, 8][new[6]]
    old[7] = [1, 2, 3, 0, 4, 5][old[7]]
    new[7] = [1, 2, 3, 0, 4, 5][new[7]]
...
def mirrow_features(old, new, action):
    ...
    old[6] = [0, 3, 2, 1, 4, 7, 6, 5, 8][old[6]]
    old[7] = [0, 3, 2, 1, 4, 5][old[7]]
    new[6] = [0, 3, 2, 1, 4, 7, 6, 5, 8][new[6]]
    new[7] = [0, 3, 2, 1, 4, 5][new[7]]
...
def best_action(features):
    ...
    if features[7] == 4 and features[0] == 1:
        return ACTIONS[5]
    if features[6] <= 3 and features[aim] <= 3 :#go to coin if one is reachable
        return ACTIONS[features[6]]
    if np.sum(features[2:6] == 1) >= 1 and features[0] == 1: #if next to a crate and fleeing from bomb is possible: set bomb
        return ACTIONS[5]
    if features[6] <= 7 and features[aim] == 0 and features[6] >=4:#go to crate if one exists
        return ACTIONS[features[6] - 4]
    if features[7] <= 3:
        return ACTIONS[features[7]]
